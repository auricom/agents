...
describe("main bootstrap", () => {
  it("bootstraps services and configures telegram webhook/commands", async () => {
    ...
    vi.doMock("node:fs/promises", () => ({ default: { mkdir } }));
    ...
    expect(mkdir).toHaveBeenNthCalledWith(1, "/workspace", { recursive: true });
    expect(mkdir).toHaveBeenNthCalledWith(2, "/tmp/sessions", { recursive: true });
